...
// func (r Router) RouterFunc(app handlers.Application) chi.Router {
// 	// Создаем chi роутер
// 	r.Router = chi.NewRouter()
// 	// Добавляем все функции middleware
// 	r.Router.Use(app.Authorization)
...
// 		r.Post("/", app.ShortenHandler) // POST запрос отправляем на сокращение ссылки
// 		r.Route("/{id}", func(r chi.Router) {
// 			r.Get("/", app.ExpandHandler) // GET запрос с id направляем на извлечение ссылки
// 		})
// 		r.Route("/api/shorten", func(r chi.Router) {
// 			r.Post("/", app.ShortenJSONHandler)           // POST запрос с JSON телом
// 			r.Post("/batch", app.ShortenBatchJSONHandler) // POST запрос с множественным JSON телом
// 		})
// 		r.Route("/api/user", func(r chi.Router) {
// 			r.Get("/urls", app.ExpandUserURLSHandler)    // GET запрос на выдачу всех сокращенных ссылок пользователем
// 			r.Delete("/urls", app.DeleteUserURLSHandler) // DELETE запрос удаляет ссылки перечисленные в запросе
// 		})
// 		r.Route("/ping", func(r chi.Router) {
// 			r.Get("/", app.CheckDBHandler) // GET запрос проверяет работоспособность базы данных
// 		})
// 	})
// 	return r.Router
// }
